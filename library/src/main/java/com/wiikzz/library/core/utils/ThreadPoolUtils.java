...
public class ThreadPoolUtils {
    ...
    public enum Type {
        FixedThread,    // 构造一个固定线程数目的线程池
        CachedThread,   // 构造一个可缓存线程池，如果线程池长度超过处理需要，可灵活回收空闲线程，若无可回收，则新建线程。
        SingleThread,   // 构造一个只支持一个线程的线程池，相当于newFixedThreadPool(1)
    }
    ...
    public ThreadPoolUtils(Type type, int corePoolSize) {
        ...
        switch (type) {
            ...
            case SingleThread:
                ...
                break;
            case CachedThread:
                // 构造一个可缓存线程池，如果线程池长度超过处理需要，可灵活回收空闲线程，若无可回收，则新建线程。
